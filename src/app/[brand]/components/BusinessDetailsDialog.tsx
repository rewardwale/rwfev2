...
const BusinessDetailsDialog: React.FC<BusinessDetailsDialogProps> = ({ business }) => {
  return (
    <div className="h-[90vh] md:h-[80vh] w-full flex flex-col">
      <div className="bg-gray-800 text-white p-4">
        <h2 className="text-xl md:text-2xl font-bold">{business.businessName}</h2>
      </div>
      
      <Tabs defaultValue="overview" className="flex-1 flex flex-col overflow-hidden pb-5">
        <TabsList className="w-full justify-start px-2 md:px-4 py-0 h-12 bg-gray-100 border-b">
          <TabsTrigger 
            value="overview" 
            className="data-[state=active]:bg-white data-[state=active]:border-b-2 data-[state=active]:border-gray-800 rounded-none px-3 md:px-4"
          >
            Overview
          </TabsTrigger>
          <TabsTrigger 
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                {Object.entries(business.content || {}).map(([key, values]) => (
                  <div key={key} className="bg-gray-50 p-4 rounded-lg overflow-hidden">
              ...
              <div>
                <h3 className="text-lg md:text-xl font-semibold mb-3">Contact Information</h3>
                <div className="space-y-3">
                  ...
                  <p className="flex items-center gap-2">
                    ...
                    <a 
                      ...
                      className="text-blue-600 hover:underline"
                    >
                      {business.contactUsDetails.indCountryCode} {business.contactUsDetails.indMobileNum}
                    </a>
                  </p>
              ...
              <div>
                <h3 className="text-lg md:text-xl font-semibold mb-3">Social Media</h3>
                <div className="flex flex-wrap gap-3">
                  {business.socialUrls.whatsapp && (
                    <a 
                      ...
                      className="px-4 py-2 bg-green-500 text-white rounded-md hover:bg-green-600 transition-colors text-sm md:text-base"
                    >
                      WhatsApp
                    </a>
                  )}
                  {business.socialUrls.instagram && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="px-4 py-2 bg-purple-500 text-white rounded-md hover:bg-purple-600 transition-colors text-sm md:text-base"
                    >
                      Instagram
                    </a>
                  )}
                  ...
                  {business.socialUrls.twitter && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="px-4 py-2 bg-blue-400 text-white rounded-md hover:bg-blue-500 transition-colors text-sm md:text-base"
                    >
                      Twitter
                    </a>
                  )}
                  {business.socialUrls.linkedin && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="px-4 py-2 bg-blue-700 text-white rounded-md hover:bg-blue-800 transition-colors text-sm md:text-base"
                    >
                      LinkedIn
                    </a>
                  )}
          ...
          <TabsContent value="photos" className="h-full p-4 md:p-6 m-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {business.businessBanners.map((banner, index) => (
                <div key={`banner-${index}`} className="aspect-square rounded-lg overflow-hidden">
                  <img 
                    src={banner.original} 
          ...
          <TabsContent value="hours" className="h-full p-4 md:p-6 m-0">
            <div className="space-y-4">
              <h3 className="text-lg md:text-xl font-semibold mb-4">Operational Hours</h3>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {Object.entries(business.operationalHours).map(([day, hours]) => (
                        <tr key={day} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                            {hours.map((hour, index) => (
                              <div key={index}>
                                {formatHours(hour.open)} - {formatHours(hour.close)}
                              </div>
                            ))}
                          </td>
